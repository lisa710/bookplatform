...
//        }
    ...
    public function inforedit()
    {
        ...
        if ($ji_no >= $mhinfo["pay_num"] && $mhinfo['free_type'] == 2 && $this->user['vip'] == 0) { //如果集大于付费级别
            if (!$buy_record) {
                redirect(U('Mh/buy', ['type' => 'mh', 'rid' => $mhid, 'ji_no' => $ji_no]));
            }
        ...
        switch ($buy_type) {
            case 1://购买当前章节
                $buy_detail = M('buy_order as bo')
                    ...
                    ->where(['bo.user_id' => $this->user['id'], 'book_type' => $type, 'bo.buy_type' => 1, 'bo.rid' => $rid, 'bd.episodes' => $current])
                    ->find();
                ...
                if ($type == 'mh') {
                    ...
                    $info = M('book as b')->field('b.member_id,be.*')
                        ->join('vv_book_episodes be ON be.bid = b.id', 'left')
                        ->where("b.id = {$rid} AND be.ji_no = {$current}")
                        ->find();
                }
            ...
            case 2:
                $buy_detail = M('buy_order as bo')
                    ->field('episodes')
                    ->join('vv_buy_detail bd ON bd.order_id = bo.id', 'left')
                    ->where(['bo.user_id' => $this->user['id'], 'book_type' => $type, 'bo.buy_type' => 1, 'bo.rid' => $rid])
...
//                }
                ...
                if ($type == 'mh') {
                    ...
                    $info = M('book as b')->field('b.member_id,be.*')
                        ->join('vv_book_episodes be ON be.bid = b.id', 'left')
                        ->where("b.id = {$rid} AND be.ji_no <= {$ji_end} AND be.ji_no >= $ji_start")
                        ->select();
                }
            ...
            case 3://整本购买
                if ($type == 'mh') {
                    $info = M('mh_list')->where("id = {$rid}")->find();
                } else {
                    $info = M('book')->where("id = {$rid}")->find();
                }
    ...
    public function getMoney()
    {
        ...
        $this->assign('current_user_id', $this->user['id']);
        $this->assign('type', $type);
        $html = $this->fetch();
        if (!$list) {
            $this->error($html);
    ...
    public function myTeam()
    {
        ...
        $this->assign('m1', sprintf("%.2f", $m1));
        $this->assign('m2', sprintf("%.2f", $m2));
        $this->assign('parent', $parent);
        $this->assign($asdata);
        $this->display();
    }
    ...
    public function pwd()
    {
        if (IS_POST) {
            ...
            if (!preg_match('/^\w{6,10}$/', $pwd1)) {
                echo json_encode(array('status' => 0, 'info' => '请输入6-10位字母数字组合！'));
                exit;
            }
            if ($pwd1 != $pwd2) {
                echo json_encode(array('status' => 0, 'info' => '两次密码输入不一致！'));
                exit;
            }

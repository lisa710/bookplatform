...
<div class="navbar flt">
    <nav class="tab-box">
		<div class="item">
            <a href="<?php echo U('Mh/index',array('parent'=>$user[id]));?>" class="active">漫画</a>
        </div>
        <div class="item">
            <a href="<?php echo U('Book/index',array('parent'=>$user[id]));?>">小说</a>
        </div>
		 <div class="item">
            <a href="<?php echo U('Yook/index',array('parent'=>$user[id]));?>">听书</a>
        </div>
    </nav>
    <div class="action">
        <a href="<?php echo U('Mh/load_search',array('parent'=>$user[id]));?>" class="btn">
...
<div id="banner-container">
    ...
    <div class="indexnav portal-slick mt-navbar">
        <?php if(is_array($_banner['config'])): $i = 0; $__LIST__ = $_banner['config'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item">
				<a href="<?php echo ($vo["url"]); ?>&parent=<?php echo ($user["id"]); ?>">
...
<nav class="portal-nav">
	<?php if(is_array($_mhcate)): foreach($_mhcate as $k=>$v): if($v['show'] == 1 and $v['isshow']): ?><div class="item">
			<a href="<?php echo ($v["url"]); ?>&parent=<?php echo ($user["id"]); ?>">
...
<div class="tabar flb">
    <nav class="nav hls1">
        ...
        <div class="item">
            <a href="<?php echo U('Mh/my',array('parent'=>$user[id]));?>">
